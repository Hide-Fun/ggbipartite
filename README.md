...
この README は、`ggbipartite`
を使って「二部ネットワーク＋系統樹」を段階的に可視化するためのガイドです。
...
まず必要なパッケージを読み込み、植物側・菌側の系統樹を作成します。次に、各系統樹の
tip 順に合わせて相互作用行列を並べ替え、可視化に使う long 形式データと
`metadata_row` を準備します。最後に、abundance 表示の基本プロット `p`
と座標計算結果 `bn_coords` を作成します。

この節で作成する主なオブジェクトは以下のとおりです。

- `interaction_matrix`: 描画の基準となる行列データ
- `interaction_df`: `ggplot` に直接渡す long 形式データ
- `metadata_row`: `row_box` 側の色分けに使う行メタデータ
...
`compute_box_label_coords()` で row/column
側の箱データからラベル用の代表座標を計算し、`annotate()` で左右に host /
otu のラベルを付与します。

この関数は、`xmin/xmax/ymin/ymax` から side ごとの外側 x 座標（row は
`xmin`、column は `xmax`）と中心 y
...
label_df_row <- compute_box_label_coords(
  .box = row_box,
  .by = "row",
  .side = "row"
)

label_df_column <- compute_box_label_coords(
  .box = column_box,
  .by = "column",
  .side = "column"
)
...
p +
  annotate(
    geom = "text",
    x = label_df_row$x - 100,
    y = label_df_row$y,
    label = label_df_row$row
  ) +
  annotate(
    geom = "text",
    x = label_df_column$x + 100,
    y = label_df_column$y,
    label = label_df_column$column
  ) +
  theme_void()
```
...
`adjust_tree()` で左右の系統樹を箱の高さに合わせ、`create_link()`
で箱の中心と tip
をつなぐ点線リンクを生成します。最後に、tree・link・network・link・tree
の 5 パネルを横一列に並べて出力します。

この構成の目的は、ネットワーク側のノードと系統樹の tip
の対応関係を視覚的に明示することです。`get_yrange()` で y
範囲を揃える処理を共通化しているため、左右でスケールがずれることを防げます。
...
`adjust_tree()` の結果に対し、`.tiplab_segment_data()` と
`.tiplab_marquee_data()`
を使ってラベル位置を整列させながら補助セグメントを引く版です。パネル構成は
3 節と同じ 5 パネルで比較できます。

3 節との主な違いは tip
ラベルの描画方法です。ラベルを揃えたまま補助線を引くことで可読性が向上するため、ラベルの見やすさを重視したい場面で特に有効です。
...
相互作用行列を 0/1 に変換し、`interaction_type = "binary"`
で二値の相互作用を描画します。系統樹とリンク線の組み合わせは abundance
版と同様です。

abundance 表示では相互作用量が面積や厚みに反映されますが、binary
表示では「存在するかどうか」だけに情報を絞ります。強度の差よりも接続パターンの有無を重視したい解析に向いています。
...
補助線をなくすことで図がすっきりしますが、どの tip
とどのノードが対応するかが一目ではわかりにくくなります。対象数が多い場合は、5
節のリンク線あり版の方が読みやすいこともあるため、目的に応じて使い分けてください。
...
に渡すことで、binary ノードの位置を tree スケールに厳密に対応させます。

6
節が整列表示を優先するのに対し、この節では系統樹の元スケールを維持したまま座標を対応させる点が主眼です。比較する系統樹に意味のある枝長情報が含まれる場合は、このアプローチの方が解釈しやすくなります。
