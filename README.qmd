...
The coordinates can be obtained with construct_bn_coordination(), so I’ll use that for adding labels.
...
host_phylo <- rtree(nrow(interaction_matrix))
host_phylo$tip.label <- LETTERS[1:10]
## symbiont (fungal) phylogeny.
symbiont_phylo <- rtree(ncol(interaction_matrix))
...
interaction_matrix <- tibble(
  ...
  mutate(
    host = factor(
      host,
      levels = c("J", "I", "H", "G", "E", "D", "F", "B", "A", "C")
    )
  ) %>%
  arrange(host) %>%
  relocate(host, otu3, otu2, otu1, otu4) %>%
  column_to_rownames("host") %>%
  as.matrix()
...
interaction_df <-
  ...
  filter(num_seq > 0) %>%
  mutate(
    host = factor(host, rownames(interaction_matrix)),
    otu = factor(otu, colnames(interaction_matrix))
  )
...
new_nm <- tibble(
  old = LETTERS[1:10],
  new = glue::glue("***{old}*** *{letters[1:10]}* test")
)
...
host_phylo2 <- adjust_tree(
  ...
  marquee_tiplab(
    ...
    show_guides = TRUE,
    x_start_nudge = .2,
    guide_end_gap_ratio = .03,
    show_labels = FALSE
  )

symbiont_phylo2 <- adjust_tree(
...
create_link <- function(box, ggtree, direction, xmin = 0, xend = 1) {
  box <- box %>% 
    pivot_longer(
      cols = c(xmin, xmax, ymin, ymax),
      names_to = c("axis", "end"),
    ...
    # Spread axis -> get single columns x and y
    pivot_wider(
      names_from = axis,
      values_from = value
    ) %>% 
...
df_link1 <- plot_link(bn_coords_box = bn_coords$box1, ggtree = host_phylo2)
df_link2 <- plot_link(bn_coords_box = bn_coords$box2, ggtree = symbiont_phylo2)

p_link1 <- ggplot() +
  geom_segment(
    data = df_link1,
    mapping = aes(x = x, y = y, xend = xend, yend = yend, group = label),
    linetype = "dotted",
    linewidth = .3
  ) +
  geom_point(
