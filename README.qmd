...
`compute_box_label_coords()` で row/column 側の箱データからラベル用の代表座標を計算し、`annotate()` で左右に host / otu のラベルを付与します。

この関数は、`xmin/xmax/ymin/ymax` から side ごとの外側 x 座標（row は `xmin`、column は `xmax`）と中心 y 座標を集約して返します。ラベル描画時は、箱の外側へさらにオフセットして重なりを防いでいます。
