...
- `ggbipartite::geom_tipmarquee()`: tip ラベルを同一フォントで安定描画します。
- `ggbipartite::geom_nodemarquee()`: node support ラベルを同じ描画系へ揃えます。

以降のコードでは、関数を定義せずに各チャンク内で直接
`adjust_tree()` / `geom_tipmarquee()` / `geom_nodemarquee()` / `geom_treescale()`
を組み合わせます。
