...
vignette: >
  %\VignetteIndexEntry{ggbipartiteで作るPublishableな系統樹×二者ネットワーク図}
  %\VignetteEngine{quarto::html}
  %\VignetteEncoding{UTF-8}
---

この vignette では、`ggbipartite` と `ggtree` を組み合わせて、
投稿図として使いやすい品質の図を作る発展例を示します。

扱う内容:

- `align = TRUE` の tip 表示
- `geom_treescale()` によるスケール表示
- `geom_nodelab()` による bootstrap 値表示
- `marquee` による tip / node ラベルの文字スタイル統一
- abundance 版と binary 版（系統樹スケール維持）の両方
- `RPANDA::phylosignal_sub_network()` 結果を `geom_nodepoint()` で重ねる例

## 事前準備

この章で使う主な関数:

- `base::library()`: 必要パッケージを読み込みます。
- `ggbipartite` の座標・描画関数は次章以降で使用します。
...
- `ape::rtree()`: 例示用の系統樹を作成します。
- `ggbipartite::get_tip_order()`: 系統樹と行列の並び順を揃えます。
- `ggbipartite::to_longer()`: 行列を `row-column-count` 形式へ変換します。
...
- `ggtree::geom_tiplab(align = TRUE)`: tip ラベルを整列表示します。
- `ggtree::geom_treescale()`: 枝長スケールを図に追加します。
- `ggtree::geom_nodelab()`: 内部ノード bootstrap 値を表示します。
...
host_tree_base <- ggtree::ggtree(host_phylo)
...
- `ggbipartite::adjust_tree()`: tree を bipartite ボックスへ整列します。
- `ggbipartite::.tiplab_segment_data()`: align 用の補助線座標を作ります。
- `ggbipartite::.tiplab_marquee_data()`: `geom_marquee()` 用 tip 座標を作ります。
- `marquee::geom_marquee()`: tip ラベルを同一フォントで安定描画します。

以降のコードでは、関数を定義せずに各チャンク内で直接
`adjust_tree()` / `geom_marquee()` / `geom_nodelab()` / `geom_treescale()`
を組み合わせます。
...
- `ggbipartite::construct_bn_coordination()`: row / column / interaction 座標を作成します。
- `ggbipartite::geom_bipnet_box()`: row / column ボックスを描きます。
- `ggbipartite::geom_bipnet_interaction()`: abundance ポリゴンを描きます。
- `ggbipartite::create_link()`: tree と box を結ぶリンク座標を作ります。
- `ggbipartite::get_yrange()`: パネル間の y 軸を統一します。
...
t_host_abundance <- t_host_abundance_base +
  geom_segment(
    data = function(df) ggbipartite::.tiplab_segment_data(df, offset = 0),
  ...
  marquee::geom_marquee(
    data = function(df) ggbipartite::.tiplab_marquee_data(df, offset = 0),
...
二値相互作用を線分として描画します。
- `ggbipartite::geom_bipnet_point()`: row / column ノードを点で描画します。
- `tip_positions_row` / `tip_positions_column`:
系統樹由来の y 座標を直接ネットワークへ適用します。
- `metadata_row` + `after_stat(host_lineage)`:
binary でも host lineage による色分けが可能です。
- `ggtree::ggtree()`: 枝長スケールを保持した tree パネルを構築します。
...
t_host_binary_base <- ggtree::ggtree(host_phylo)
...
t_symbiont_binary_base <- ggtree::ggtree(symbiont_phylo)
...
- `RPANDA::phylosignal_sub_network()`:
clade 単位の相関検定を実行する関数（本章では実行せず参照のみ）。
- `tibble::tribble()`:
デモ用の clade シグナル表をコード内で直接作成します。
- `ggtree::geom_nodepoint()`:
検定結果（`mantel_cor`）を内部ノードへ重ね描きします。
...
cophylo_by_clade_plants <- tibble::tribble(
...
cophylo_by_clade_fungi <- tibble::tribble(
  ~data_source, ~node, ~nb_A, ~nb_B, ~mantel_cor, ~pvalue_upper, ~pvalue_lower, ~pvalue_upper_corrected, ~pvalue_lower_corrected,
...
t_host_binary_signal <- t_host_binary %<+%
  cophylo_by_clade_plants_sigif_upper +
  ggtree::geom_nodepoint(aes(size = mantel_cor, colour = mantel_cor)) +
  common_scale +
  theme(
    legend.position = "bottom",
